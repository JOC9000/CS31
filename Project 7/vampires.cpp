...
void Vampire::move()
{
	...
	if (isDrunk && turnsSince % 2 == 1)
		turnsSince++;
	else
	{
		int n = randInt(0, 3);
		if (attemptMove(*m_arena, n, m_row, m_col))
		{
...
Arena::~Arena()
{
	...
	for (int num = 0; num < vampireCount(); num++)
		delete m_vampires[num];
...
int Arena::numberOfVampiresAt(int r, int c) const
{
	...
	for (int k = 0; k < vampireCount(); k++)
	{
		...
		if (r == m_vampires[k]->row() && c == m_vampires[k]->col())//may be wrong
			totalVamps++;
	}

	return totalVamps;  // This implementation compiles, but is incorrect.
}
...
void Arena::display(string msg) const
{
	...
	for (r = 1; r <= rows(); r++)
		for (c = 1; c <= cols(); c++)
		{
			if (numberOfVampiresAt(r, c) > 1 && numberOfVampiresAt(r, c) < 9)//fix
			{
...
void Arena::moveVampires()
{
	...
	for (int k = 0; k < vampireCount(); k++)
	{
		...
		if (m_player != nullptr && m_vampires[k]->col() == m_player->col() && m_vampires[k]->row() == m_player->row() && !m_vampires[k]->isDead())
			m_player->setDead();

		if (m_vampires[k]->isDead())
		{
			delete m_vampires[k];

			for (int l = 0; l + k + 1 < vampireCount(); l++)//may be error
				m_vampires[k + l] = m_vampires[k + l + 1];
...
bool attemptMove(const Arena& a, int dir, int& r, int& c)
{
	...
	if (tmpRow > 0 && tmpRow <= a.rows() && tmpCol > 0 && tmpCol <= a.cols())
	{
		r = tmpRow;
		c = tmpCol;
		return true;
...
bool isMoveSafe(const Arena& a, int r, int c)
{

	if ((r+1 > a.rows() || a.numberOfVampiresAt(r + 1, c) == 0) && (c+1 > a.cols() || a.numberOfVampiresAt(r, c + 1) == 0) && (r-1 < 1 || a.numberOfVampiresAt(r - 1, c) == 0) && (c-1 < 1 ||a.numberOfVampiresAt(r, c - 1) == 0))
		return true;
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
	...
	if (r - 1 > 0 && a.numberOfVampiresAt(r - 1, c) == 0)
	{
		if (isMoveSafe(a, r-1, c))
		{
			bestDir = 0;
	...
	if (c - 1 > 0 && a.numberOfVampiresAt(r, c-1) == 0)
	{
		if (isMoveSafe(a, r, c-1))
		{
			bestDir = 3;
